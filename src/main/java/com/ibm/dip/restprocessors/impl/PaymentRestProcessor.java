...
@Service
public class PaymentRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(PaymentRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.payment.hosturl}")
	private String url;
	
	@Value("${spring.api.payment.context-path}")
	private String contextPath;

	@Value("${spring.api.payment.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  PaymentRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ PaymentRestProcessor.class.getName());
